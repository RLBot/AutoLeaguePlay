...
def run_match(
    ...
    with setup_manager_context() as setup_manager:
        ...
        for exercise_result in run_playlist(
            ...
            if (
                isinstance(exercise_result.grade, Fail)
                ...
                if session_result_path.exists():
                    ...
                    if historical_result is not None:
                        ...
                        match_config = make_match_config(
                            participant_1.bot_config,
                            participant_2.bot_config,
                            team_size,
                        )
                        result = run_match(
                            participant_1.bot_config.name,
...
def get_round_robin_ranges(ladder, div_index, half_robin):
    ...
    if half_robin:
        ...
        upper_range = (
            div_index * ladder.division_size,
            div_index * ladder.division_size + num_bots // 2,
        )
        lower_range = (
            div_index * ladder.division_size + num_bots // 2,
            (div_index + 1) * ladder.division_size,
        )
        sub_ranges = [lower_range, upper_range]
    else:
        sub_ranges = [
            (div_index * ladder.division_size, (div_index + 1) * ladder.division_size)
        ]
    return sub_ranges


def find_historical_result(
    bot1: VersionedBot,
    bot2: VersionedBot,
    ...
    return get_stale_match_result(
        bot1, bot2, stale_rematch_threshold, working_dir, True
    )
...
def get_stale_match_result(
    ...
    if stale_rematch_threshold > 0:
        match_history = MatchHistory(
            working_dir.get_version_specific_match_files(bot1.get_key(), bot2.get_key())
        )
