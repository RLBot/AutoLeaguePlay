...
GAME_TICK_PACKET_REFRESHES_PER_SECOND = (
    120
)  # 2*60. https://en.wikipedia.org/wiki/Nyquist_rate
...
def frame_tick():
    ...
    if packet.game_info.is_match_ended:
        ...
        if (
            not obs.obs_frontend_recording_active()
            and start_time == 999999999999999
            and auto_record
        ):
...
def boost_bar(boost):
    ...
    obs.vec2_set(
        pos_vec,
        boost_bar_pos[0] + boost_bar_size[0] * (1 - perc_bar[0]),
        boost_bar_pos[1],
    )
                ...
                for item in items:
                    if item is not None:
                        ...
                        if obs.obs_source_get_name(scene_source) == name:
                            sceneItem = obs.obs_scene_find_sceneitem_by_id(
                                scene, obs.obs_sceneitem_get_id(item)
                            )
                            obs.source_list_release(scenes)
...
def set_text_pos():
    ...
    pos_vec = obs.vec2()
    obs.vec2_set(
        pos_vec, 1920 - pos[0] - hardcoded_width * scale[0] + bit_to_right, pos[1]
    )
    obs.obs_sceneitem_set_pos(sceneItem, pos_vec)
    scale2_vec = obs.vec2()
    obs.vec2_set(scale2_vec, scale[0], scale[1])
    ...
    obs.vec2_set(
        pos_vec,
        1920 - pos[0] - hardcoded_width * scale[0] + bit_to_right,
        pos[1] + hardcoded_height * scale[1],
    )
    obs.obs_sceneitem_set_pos(sceneItem, pos_vec)
    scale2_vec = obs.vec2()
    obs.vec2_set(scale2_vec, scale[0], scale[1])
...
def auto_setup(props, prop):  # Todo: add BO3 and BO5
    ...
    if not check_for_scene("RLBot - AutoLeague"):
        ...
        social_source = obs.obs_source_create(
            "image_source", "Social", temp_settings, None
        )
        ...
        obs.obs_data_set_string(temp_settings, "file", temp_path)
        overlay_source = obs.obs_source_create(
            "image_source", "Overlay", temp_settings, None
        )
        ...
        blue_name_source = obs.obs_source_create(
            "text_gdiplus", "Blue-Name", temp_settings, None
        )
        ...
        orange_name_source = obs.obs_source_create(
            "text_gdiplus", "Orange-Name", temp_settings, None
        )
        ...
        blue_dev_name_source = obs.obs_source_create(
            "text_gdiplus", "Blue-Dev-Name", temp_settings, None
        )
        ...
        orange_dev_name_source = obs.obs_source_create(
            "text_gdiplus", "Orange-Dev-Name", temp_settings, None
        )
        ...
        obs.obs_data_set_string(temp_settings, "file", temp_path)
        logo_0_source = obs.obs_source_create(
            "image_source", "Logo-0", temp_settings, None
        )
        ...
        obs.obs_data_set_string(temp_settings, "file", temp_path)
        logo_0_source = obs.obs_source_create(
            "image_source", "Logo-1", temp_settings, None
        )
        logo_0_item = obs.obs_scene_add(main_scene, logo_0_source)
        obs.obs_data_release(temp_settings)
        obs.obs_source_release(logo_0_source)
        ...
        goal_source = obs.obs_source_create(
            "ffmpeg_source", "Goal", temp_settings, None
        )
